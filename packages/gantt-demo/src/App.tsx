...
import { GanttHistory, type GanttEngine, type GanttId, type TaskChange } from '@gantt-chart/core';
...
import { generate, statusColor, type DemoGroupData, type DemoTask, type DemoTaskData } from './data';
...
export function App(): JSX.Element {
  ...
  const dataset = useMemo(() => generate({ taskCount }), [taskCount]);
  ...
  const options = useMemo(
    () => ({
      stacking: { enabled: stacking, rollupCollapsed: rollup },
      interaction: { snapMs },
    }),
    [stacking, rollup, snapMs],
  );
  ...
  return (
    <div className={`app${dark ? ' app--dark' : ''}`}>
      <header className="app__bar">
        ...
        <Toggle label="Roll up" checked={rollup} onChange={setRollup} title="Show hidden children on their collapsed ancestor" />
      ...
      <div className="app__chart">
        <GanttChart<DemoTaskData, DemoGroupData>
          ...
          engineRef={setEngine}
          headerCorner={<span>{dataset.groups.length.toLocaleString()} rows</span>}
        />
      </div>
      ...
      <footer className="app__stats">
        ...
        {stats.truncated ? <span className="app__warn">frame truncated by maxVisibleItems</span> : null}
        <span className="app__hint">
          drag bars · drag edges to resize · marquee on empty space · wheel scrolls · ctrl+wheel zooms ·
